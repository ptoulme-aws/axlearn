...
class AccumulatedLearnerTest(TestCase):
    def test_prune_empty_state(self):
        state = {
            "state": {
                "tensor": jnp.array(0),
    ...
    def test_learner(self, ema_decay: Optional[float], method: str):
        ...
        if method == "update":
            ...
            inputs = dict(fn=loss_fn, inputs={"input_batches": jnp.ones((4, 8))}, opt_params=params)
        else:
            raise NotImplementedError
    ...
    def test_update_rules(self, update_rules):
        ...
        cfg = AccumulatedLearner.default_config().set(
            name="test", optimizer=optimizer_cfg, update_rules=update_rules, microbatches=4
        )
        learner: AccumulatedLearner = cfg.instantiate(parent=None)
        ...
        loss, grads = jax.value_and_grad(loss_fn)(jax.tree_util.tree_map(lambda p: p.value, params))
        np.testing.assert_allclose(loss, 1.412078, atol=1e-6, rtol=1e-4)
        self.assertNestedAllClose(
            dict(v=jnp.asarray([0.089629, -0.756364, 0.662272, 0.004462]), c=0.0),
            grads,
            atol=1e-6,
            rtol=1e-4,
        )
    ...
    def test_update_rules_on_vdict(self, update_rules):
        ...
        self.assertNestedAllClose(dict(v=expected_new_v), updated_params, atol=1e-6, rtol=1e-4)
    ...
    def test_per_variable_summaries(self):
        ...
        cfg = AccumulatedLearner.default_config().set(
            name="test", optimizer=optimizer_cfg, enable_per_variable_summaries=True, microbatches=4
        )
        learner: AccumulatedLearner = cfg.instantiate(parent=None)
        params = dict(
            weight=OptParam(
